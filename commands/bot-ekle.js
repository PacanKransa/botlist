...
module.exports = {
	...
	run: async(client, message, args) => {
     ...
     .setColor("BLUE")
      .setDescription("Lütfen Eklemek İstediğiniz Botun ID sini Giriniz.")
     .setAuthor(message.author.username, message.author.avatarURL({dynamic: true}))
	 .setTimestamp()
     .setFooter(client.user.username, client.user.avatarURL())
      ...
      try {
          ...
          return message.channel.send(embed.setDescription("Discord Apide Böyle Bir Bot Bulamadım.")).then(msg => msg.delete({timeout:3000}));
	  }		

	  if(!discordBot.bot) return message.channel.send(embed.setDescription("Lütfen Bot IDsi Giriniz, Kullanıcı ID Girmeyin!")).then(msg => msg.delete({timeout:3000}));
